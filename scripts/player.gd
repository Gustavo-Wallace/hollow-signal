extends Node2D

const MAX_SPEED := 330.0
const ACCELERATION := 1250.0
const DECELERATION := 1650.0
const QUICK_COOLDOWN := 0.3
const RESONANT_COOLDOWN := 0.62
const BREAK_COOLDOWN := 1.25
const CHARGE_FULL_TIME := 0.86
const QUICK_MAX_CHARGE := 0.24
const RESONANT_MAX_CHARGE := 0.79
const MAX_CHARGE_GRACE := 0.4
const MAX_CHARGE_EXPOSURE_RATE := 0.04
const MAX_CHARGE_TRACE_RATE := 0.2
const MAX_HEALTH := 3
const INVULNERABILITY_DURATION := 1.0
const EXPOSURE_DECAY := 0.05
const EXPOSURE_DECAY_DELAY := 1.15
const NULL_POCKET_DECAY_MULTIPLIER := 3.0
const TRACE_MOVE_THRESHOLD := 72.0
const TRACE_WINDOW := 2.4
const ARENA_BOUNDS := Rect2(62.0, 62.0, 1156.0, 556.0)

var velocity := Vector2.ZERO
var trail_points: PackedVector2Array = []
var trail_ages: PackedFloat32Array = []
var trail_gradient: Gradient
var pulse_cooldown := 0.0
var health := MAX_HEALTH
var invulnerability_time := 0.0
var exposure := 0.0
var exposure_decay_delay := 0.0
var damage_flash := 0.0
var destroyed := false
var trace_anchor := Vector2.ZERO
var trace_time := 0.0
var trace_value := 0.0
var trace_locked := false
var is_charging := false
var charge_ratio := 0.0
var max_charge_hold_time := 0.0
var current_cooldown_length := QUICK_COOLDOWN
var null_suppressed := false
var signal_release_required := false
var core_saturated := false
var break_limit_reported := false
var last_signal_profile := "quick"
@onready var trail: Line2D = $Trail
@onready var signal_hint: Label = $"../Interface/SignalHint"


func _ready() -> void:
	process_mode = Node.PROCESS_MODE_PAUSABLE
	add_to_group("signal_player")
	trail_points.append(global_position)
	trail_ages.append(0.0)
	trail_gradient = Gradient.new()
	trail_gradient.offsets = PackedFloat32Array([0.0, 0.55, 1.0])
	trail_gradient.colors = PackedColorArray([Color(0.24, 0.9, 1.0, 0.5), Color(0.18, 0.68, 0.94, 0.22), Color(0.1, 0.38, 0.62, 0.0)])
	trail.gradient = trail_gradient
	trace_anchor = global_position
	queue_redraw()


func _process(delta: float) -> void:
	if destroyed or not get_parent().is_playing():
		_cancel_charge()
		return
	_move(delta)
	_update_trail(delta)
	null_suppressed = get_parent().is_player_in_null_pocket(global_position)
	if null_suppressed:
		if is_charging:
			_cancel_charge()
		if Input.is_action_pressed("ui_accept"):
			signal_release_required = true
	pulse_cooldown = maxf(0.0, pulse_cooldown - delta)
	exposure_decay_delay = maxf(0.0, exposure_decay_delay - delta)
	invulnerability_time = maxf(0.0, invulnerability_time - delta)
	var progress_decay := lerpf(1.0, 0.62, get_parent().get_progress_ratio())
	if get_parent().is_escape():
		progress_decay = 0.2
	if exposure_decay_delay <= 0.0:
		var decay_multiplier := NULL_POCKET_DECAY_MULTIPLIER if null_suppressed else 1.0
		exposure = maxf(0.0, exposure - EXPOSURE_DECAY * progress_decay * decay_multiplier * delta)
	_update_core_saturation()
	damage_flash = maxf(0.0, damage_flash - delta * 3.5)
	_update_signal_charge(delta)
	_update_stationary_trace(delta)
	_update_signal_hint()
	queue_redraw()


func _move(delta: float) -> void:
	var direction := Input.get_vector("move_left", "move_right", "move_up", "move_down")
	var charge_speed_multiplier := lerpf(1.0, 0.68, charge_ratio) if is_charging else 1.0
	if direction != Vector2.ZERO:
		velocity = velocity.move_toward(direction * MAX_SPEED * charge_speed_multiplier, ACCELERATION * delta)
	else:
		velocity = velocity.move_toward(Vector2.ZERO, DECELERATION * delta)
	global_position = (global_position + velocity * delta).clamp(ARENA_BOUNDS.position, ARENA_BOUNDS.end)


func _update_trail(delta: float) -> void:
	for index in trail_ages.size():
		trail_ages[index] += delta
	if trail_points.is_empty() or trail_points[0].distance_to(global_position) > 5.0:
		trail_points.insert(0, global_position)
		trail_ages.insert(0, 0.0)
	while not trail_ages.is_empty() and trail_ages[trail_ages.size() - 1] > 0.68:
		trail_ages.remove_at(trail_ages.size() - 1)
		trail_points.remove_at(trail_points.size() - 1)
	while trail_points.size() > 26:
		trail_ages.remove_at(trail_ages.size() - 1)
		trail_points.remove_at(trail_points.size() - 1)
	trail.points = trail_points
	var alpha := 0.18 + minf(velocity.length() / MAX_SPEED, 1.0) * 0.3 + exposure * 0.12
	trail_gradient.colors = PackedColorArray([Color(0.24, 0.9, 1.0, alpha), Color(0.18, 0.68, 0.94, alpha * 0.42), Color(0.1, 0.38, 0.62, 0.0)])


func _update_signal_charge(delta: float) -> void:
	if null_suppressed:
		return
	if signal_release_required:
		if not Input.is_action_pressed("ui_accept"):
			signal_release_required = false
		return
	if Input.is_action_just_pressed("ui_accept") and pulse_cooldown <= 0.0:
		is_charging = true
		charge_ratio = 0.0
		max_charge_hold_time = 0.0
		break_limit_reported = false
	if not is_charging:
		return
	if Input.is_action_pressed("ui_accept"):
		var charge_limit := RESONANT_MAX_CHARGE if core_saturated else 1.0
		charge_ratio = minf(charge_limit, charge_ratio + delta / CHARGE_FULL_TIME)
		if core_saturated and charge_ratio >= RESONANT_MAX_CHARGE and not break_limit_reported:
			break_limit_reported = true
			get_parent().break_attempt_blocked()
		if charge_ratio >= charge_limit:
			max_charge_hold_time += delta
			if max_charge_hold_time > MAX_CHARGE_GRACE:
				exposure = minf(1.0, exposure + MAX_CHARGE_EXPOSURE_RATE * delta)
				trace_value += MAX_CHARGE_TRACE_RATE * delta
				_update_core_saturation()
	elif Input.is_action_just_released("ui_accept"):
		_emit_charged_signal()


func _emit_charged_signal() -> void:
	var signal_profile := _classify_signal(charge_ratio)
	var exposure_cost := 0.04
	var trace_cost := 0.05
	match signal_profile:
		"resonant":
			exposure_cost = 0.14
			trace_cost = 0.22
			current_cooldown_length = RESONANT_COOLDOWN
		"break":
			exposure_cost = 0.34
			trace_cost = 0.72
			current_cooldown_length = BREAK_COOLDOWN
		_:
			current_cooldown_length = QUICK_COOLDOWN
	exposure = minf(1.0, exposure + exposure_cost)
	exposure_decay_delay = EXPOSURE_DECAY_DELAY
	_update_core_saturation()
	get_parent().emit_pulse(global_position, signal_profile, charge_ratio)
	pulse_cooldown = current_cooldown_length
	trace_value += trace_cost
	last_signal_profile = signal_profile
	_cancel_charge()


func _cancel_charge() -> void:
	is_charging = false
	charge_ratio = 0.0
	max_charge_hold_time = 0.0
	break_limit_reported = false


func _classify_signal(value: float) -> String:
	if value <= QUICK_MAX_CHARGE:
		return "quick"
	if value <= RESONANT_MAX_CHARGE:
		return "resonant"
	return "break"


func _update_core_saturation() -> void:
	if not core_saturated and exposure >= 0.65:
		core_saturated = true
	elif core_saturated and exposure <= 0.45:
		core_saturated = false


func _update_stationary_trace(delta: float) -> void:
	if null_suppressed:
		trace_time = 0.0
		trace_value = 0.0
		return
	var critical := exposure >= 0.7
	var moved := global_position.distance_to(trace_anchor) >= TRACE_MOVE_THRESHOLD
	if moved:
		trace_anchor = global_position
		trace_time = 0.0
		if not critical:
			trace_value = maxf(0.0, trace_value - delta * 0.32)
		if trace_locked and not critical:
			trace_locked = false
			get_parent().clear_trace_lock()
	else:
		trace_time += delta
	if critical:
		var movement_factor := lerpf(1.0, 0.42, minf(velocity.length() / MAX_SPEED, 1.0))
		trace_value += (0.28 + inverse_lerp(0.7, 1.0, exposure) * 0.22) * movement_factor * delta
	if not trace_locked and ((trace_time >= TRACE_WINDOW and trace_value >= 1.45) or (critical and trace_value >= 1.65)):
		trace_locked = true
		get_parent().activate_trace_lock(global_position)
	if not trace_locked and not critical and trace_time >= TRACE_WINDOW + 0.8:
		trace_time = 0.0
		trace_value = 0.0


func _update_signal_hint() -> void:
	if null_suppressed:
		signal_hint.text = "SIGNAL MUTED"
		signal_hint.modulate.a = 0.58
	elif core_saturated:
		signal_hint.text = "CORE SATURATED - RESONANT LIMIT"
		signal_hint.modulate.a = 0.56
	else:
		signal_hint.text = "HOLD SPACE - CHARGE SIGNAL"
		signal_hint.modulate.a = 0.95 if is_charging else 0.28 + (1.0 - pulse_cooldown / current_cooldown_length) * 0.72


func take_damage(source_position: Vector2, source_type: String = "enemy_contact") -> void:
	if invulnerability_time > 0.0 or destroyed or not get_parent().is_playing():
		return
	health -= 1
	invulnerability_time = INVULNERABILITY_DURATION
	damage_flash = 1.0
	velocity += source_position.direction_to(global_position) * 260.0
	get_parent().player_damaged(source_type)
	if health <= 0:
		destroyed = true
		get_parent().player_destroyed()
	queue_redraw()


func get_exposure() -> float:
	return exposure


func get_health() -> int:
	return health


func get_max_health() -> int:
	return MAX_HEALTH


func get_charge_profile() -> String:
	return _classify_signal(charge_ratio)


func break_trace_from_null() -> void:
	trace_time = 0.0
	trace_value = 0.0
	if trace_locked:
		trace_locked = false
		get_parent().clear_trace_lock()


func _draw() -> void:
	var pulse := sin(Time.get_ticks_msec() * 0.003) * 0.5 + 0.5
	var cooldown_progress := 1.0 - pulse_cooldown / current_cooldown_length
	var blink_alpha := 1.0 if invulnerability_time <= 0.0 else 0.38 + 0.62 * absf(sin(Time.get_ticks_msec() * 0.018))
	var charge_pulse := sin(Time.get_ticks_msec() * (0.004 + charge_ratio * 0.008)) * 0.5 + 0.5
	var null_light := 0.52 if null_suppressed else 1.0
	var recovery_flash := pulse_cooldown / BREAK_COOLDOWN if last_signal_profile == "break" and pulse_cooldown > 0.0 else 0.0
	var aura_radius := 28.0 + pulse * 3.0 + exposure * 18.0 + charge_ratio * 12.0
	draw_circle(Vector2.ZERO, aura_radius, Color(0.05, 0.5, 0.82, (0.055 + exposure * 0.14 + charge_ratio * 0.08) * blink_alpha * null_light))
	draw_circle(Vector2.ZERO, 19.0 + pulse * 2.0 + exposure * 7.0, Color(0.1, 0.72, 1.0, (0.12 + exposure * 0.16) * blink_alpha * null_light))
	draw_arc(Vector2.ZERO, 15.0, 0.0, TAU, 48, Color(0.3, 0.9, 1.0, (0.72 + exposure * 0.2) * blink_alpha), 1.2, true)
	if not null_suppressed:
		draw_arc(Vector2.ZERO, 22.0, -PI * 0.5, -PI * 0.5 + TAU * cooldown_progress, 32, Color(0.35, 0.93, 1.0, 0.52 * blink_alpha), 1.3, true)
	if is_charging:
		var charge_alpha := (0.42 + charge_ratio * 0.48) * blink_alpha
		draw_arc(Vector2.ZERO, 27.0, -PI * 0.5, -PI * 0.5 + TAU * minf(charge_ratio, QUICK_MAX_CHARGE), 18, Color(0.44, 0.94, 1.0, charge_alpha), 1.6, true)
		draw_arc(Vector2.ZERO, 27.0, -PI * 0.5 + TAU * QUICK_MAX_CHARGE, -PI * 0.5 + TAU * minf(charge_ratio, RESONANT_MAX_CHARGE), 32, Color(0.3, 0.78, 1.0, charge_alpha), 2.0, true)
		if not core_saturated:
			draw_arc(Vector2.ZERO, 27.0, -PI * 0.5 + TAU * RESONANT_MAX_CHARGE, -PI * 0.5 + TAU * charge_ratio, 18, Color(0.58, 0.96, 1.0, charge_alpha), 2.5, true)
		else:
			draw_arc(Vector2.ZERO, 31.0, -PI * 0.5 + TAU * RESONANT_MAX_CHARGE, -PI * 0.5 + TAU, 20, Color(0.22, 0.39, 0.52, charge_alpha * 0.62), 1.2, true)
		draw_arc(Vector2.ZERO, 32.0 + charge_pulse * 2.0, PI * 0.1, PI * 0.1 + TAU * charge_ratio * 0.55, 32, Color(0.3, 0.82, 1.0, charge_alpha * 0.5), 1.0, true)
	if exposure > 0.05:
		draw_arc(Vector2.ZERO, 34.0 + pulse * 5.0 + exposure * 12.0, 0.45, 2.35, 24, Color(0.3, 0.92, 1.0, exposure * 0.38 * blink_alpha), 1.0, true)
		draw_arc(Vector2.ZERO, 34.0 + pulse * 5.0 + exposure * 12.0, 3.6, 5.5, 24, Color(0.3, 0.92, 1.0, exposure * 0.38 * blink_alpha), 1.0, true)
	draw_circle(Vector2.ZERO, 10.0 + exposure * 1.5, Color(0.22, 0.83, 1.0, 0.92 * blink_alpha * null_light))
	draw_circle(Vector2.ZERO, 5.0 + damage_flash * 1.6, Color(0.8, 0.98, 1.0, blink_alpha))
	if recovery_flash > 0.0:
		draw_arc(Vector2.ZERO, 29.0 + pulse * 4.0, 0.0, TAU, 36, Color(0.5, 0.88, 1.0, recovery_flash * 0.38), 1.2, true)
	draw_line(Vector2(-17, 0), Vector2(17, 0), Color(0.35, 0.9, 1.0, 0.3 * blink_alpha), 1.0)
	draw_line(Vector2(0, -17), Vector2(0, 17), Color(0.35, 0.9, 1.0, 0.3 * blink_alpha), 1.0)
